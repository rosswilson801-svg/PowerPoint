...
const DraggableModuleCard = ({ module, children }: { module: any, children: React.ReactNode }) => {
...
const DroppableDayCell = ({ id, children, disabled }: { id: string, children: React.ReactNode, disabled?: boolean }) => {
...
const CurriculumPlanner: React.FC = () => {
    ...
    const [scheduledModules, setScheduledModules] = useState<Record<string, any>>(() => {
        if (typeof window !== 'undefined') {
            const saved = localStorage.getItem('clarity_planner_schedule');
            if (saved) {
                try {
                    const parsed = JSON.parse(saved);

                    // Handle Legacy Data (pre-versioning)
                    let version = parsed.version || 0;
                    let data = parsed.data || parsed; // Support both {version, data} and raw data

                    // MIGRATION SYSTEM
                    // This pipeline runs automatically when we update the app structure

                    if (version < 1) {
                        // Example Migration: We could fix the 0-based numbering here if we could distinguish it.
                        // For now, we just "claim" the data as Version 1 to prevent future conflicts.
                        console.log("Migrating schedule storage to v1");
                        version = 1;
                    }
    ...
    useEffect(() => {
        const payload = {
            ...
            data: scheduledModules,
            lastModified: new Date().toISOString()
        };
        localStorage.setItem('clarity_planner_schedule', JSON.stringify(payload));
    }, [scheduledModules]);
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (over && active.data.current?.module) {
            ...
            // Allow replacing existing modules
                ...
                <div className={`w-72 flex-shrink-0 bg-white border-r border-brand-primary/10 p-6 flex flex-col gap-6 shadow-sm transition-colors duration-500 ${isNordMode ? 'border-r-[#002F6C]/10' : ''}`}>
                    ...
                    <div className="flex flex-col flex-1 min-h-0">
                        ...
                        <div className="space-y-4 flex-1 overflow-y-auto pr-3 custom-scrollbar min-h-0">
                            {CLARITY_MODULES.filter(m => {
                                // Filter out modules that are already scheduled for this specific class and year
                                const isScheduled = Object.entries(scheduledModules).some(([key, scheduledModule]) => {
                                    return key.startsWith(`${activeYear}-${activeClass}-`) && (scheduledModule as any).id === m.id;
                                });
                                return !isScheduled;
                            }).map((baseModule) => {
                                                    ...
                                                    {scheduledModule ? (
                                                        ...
                                                        >
                                                            ...
                                                            <button
                                                                ...
                                                                className="absolute top-6 right-6 p-2 bg-white hover:bg-red-50 text-slate-300 hover:text-red-500 rounded-full transition-all z-50 border border-slate-200/50 hover:border-red-200"
                                                                title="Remove Lesson"
                                                            >
                                                                <Trash2 size={16} />
                                                            </button>
                                                        ...
                                                        >
                                                            ...
                                                            <button
                                                                onClick={(e) => {
                                                                    ...
                                                                    setScheduledModules(prev => {
                                                                        const next = { ...prev };
                                                                        delete next[`${activeYear}-${activeClass}-${activeMonth}-${activeWeek}-${day}`];
                                                                        return next;
                                                                    });
                                                                }}
                                ...
                                <div key={term.name} className="flex flex-col gap-4">
                                    ...
                                    <div className="grid grid-cols-6 gap-4">
                                        {Array.from({ length: 12 }).map((_, wIndex) => {
                                            // Calculate global week index approx (just for demo visual)
                                            const globalWeek = (tIndex * 12) + (wIndex + 1);
                                            // Check specifically for the PSHE day or just grab the first scheduled item for this week
                                            const targetDay = psheDay === 'All Days' ? 'Wednesday' : psheDay;

                                            // Find any scheduled module for this week/year/class
                                            // In year view, we simplifiy to showing the PRIMARY lesson for that week
                                            const rawModule = Object.entries(scheduledModules).find(([key, val]) => {
                                                const parts = key.split('-');
                                                // Key format: Year-Class-Month-Week-Day
                                                // We need to map Term Weeks to actual storage keys which are strictly indexed 0-51 globally?
                                                // Currently storage is activeYear-activeClass-activeMonth-WeekIndex-Day.
                                                // This is tricky because "Month" is state-dependent.
                                                // FIX: ideally we'd store by Global Week Number, but for now let's just 
                                                // show a visual placeholder pattern to demonstrate the VIEW.

                                                // For this demo: we just check if there's a hit for this week number in the current month loop
                                                // In a real app we'd map Term+Week -> Calendar Date -> Storage Key
                                                return val;
                                            })?.[1];

                                            // Fallback: Check strictly activeMonth (simple view)
                                            // Real implementation would calculate date ranges.

                                            return (
                                                <div key={wIndex} className="aspect-square rounded-2xl border border-brand-primary/10 bg-white p-3 flex flex-col gap-2 relative group hover:border-brand-accent/50 transition-all">
                                                    <span className="text-[9px] font-black text-brand-secondary/40 uppercase tracking-widest">W{wIndex + 1}</span>
                                                    {wIndex % 3 === 0 ? (
                                                        <div className="flex-grow bg-brand-bg rounded-xl flex items-center justify-center border border-brand-primary/5">
                                                            <div className={`w-2 h-8 rounded-full bg-brand-primary/20`} />
                                                        </div>
                                                    ) : (
